...
def dice_loss(logits, true, eps=1e-7):
    # 计算Sørensen-Dice损失。
    # 注意PyTorch优化器将损失最小化。在这个
    # 在这种情况下，我们希望骰子损失最大化，所以我们
    # 返回否定的骰子损失。
    # 参数:
    # true:形状为[B, 1, H, W]的张量。
    # logits:形状为[B, C, H, W]的张量。对应于
    # 模型的原始输出或对数。
    # Eps:加入到分母中以保持数值稳定性。
    # 返回:
    # dice_loss: Sørensen-Dice损失。
...
def jaccard_loss(logits, true, eps=1e-7):
    # 计算Jaccard损失，也就是IoU损失。
    # 注意PyTorch优化器将损失最小化。在这个
    # 情况是，我们想把雅克牌的损失最大化，所以我们
    # 返回否定的雅卡牌损失。
    # 参数:
    # true:形状为[B, H, W]或[B, 1, H, W]的张量。
    # logits:形状为[B, C, H, W]的张量。对应于
    # 模型的原始输出或对数。
    # Eps:加入到分母中以保持数值稳定性。
    # 返回:
    # jacc_loss: Jaccard的损失。
